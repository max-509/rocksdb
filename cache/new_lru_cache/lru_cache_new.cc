...
#include "cache/new_lru_cache/lru_cache_new.h"
...
LRUHandle** LRUHandleTable::FindPointer(const Slice& key, uint32_t hash) {
  LRUHandle** ptr = &list_[hash >> (std::numeric_limits<uint32_t>::digits - length_bits_)];
...
//LRUHandleTableWithOpenAddress::LRUHandleTableWithOpenAddress(int max_upper_hash_bits,
//                               MemoryAllocator* allocator)
//    : length_bits_(/* historical starting size*/ 4),
//      buf_(reinterpret_cast<LRUHandle *>(new uint8_t[sizeof(LRUHandle) * size_t{1} << length_bits_])),
//      elems_(0),
//      max_length_bits_(max_upper_hash_bits),
//      allocator_(allocator) {}
//
...
//}
//
//LRUHandle* LRUHandleTableWithOpenAddress::Lookup(const Slice& key, uint32_t hash) {
//  return *FindPointer(key, hash);
//}
//
...
//}
//
//LRUHandle* LRUHandleTableWithOpenAddress::Remove(const Slice& key, uint32_t hash) {
//  LRUHandle** ptr = FindPointer(key, hash);
//  LRUHandle* result = *ptr;
//  if (result != nullptr) {
//    *ptr = result->next_hash;
...
//}
//
//LRUHandle** LRUHandleTableWithOpenAddress::FindPointer(const Slice& key, uint32_t hash) {
